...
BASE_RUN_ID="$(toml_get base run_id "hotpotqa_gepa_public")"
...
POLICY_CONCURRENCY="${GEPA_POLICY_CONCURRENCY:-$(toml_get policy concurrency "120")}"
POLICY_RETRIES="${GEPA_POLICY_RETRIES:-$(toml_get policy retries "1")}"
POLICY_MAX_TOKENS="${GEPA_POLICY_MAX_TOKENS:-$(toml_get policy max_tokens "32")}"
...
TRAIN_SEED_START="${GEPA_TRAIN_SEED_START:-$(toml_get dataset train_seed_start "random")}"
HELDOUT_SEED_START="${GEPA_HELDOUT_SEED_START:-$(toml_get dataset heldout_seed_start "random")}"
...
for numeric_value in \
  ...
  "GEPA_POLICY_CONCURRENCY=$POLICY_CONCURRENCY" \
  "GEPA_POLICY_RETRIES=$POLICY_RETRIES" \
  "GEPA_POLICY_MAX_TOKENS=$POLICY_MAX_TOKENS" \
...
set_command_env "HOTPOTQA_POLICY_MODEL" "$POLICY_MODEL"
set_command_env "HOTPOTQA_POLICY_BASE_URL" "$POLICY_BASE_URL"
set_command_env "HOTPOTQA_POLICY_API_KEY_ENV" "$POLICY_API_KEY_ENV"
set_command_env "HOTPOTQA_POLICY_TIMEOUT_SECONDS" "$POLICY_TIMEOUT_SECONDS"
set_command_env "HOTPOTQA_ROLLOUT_TIMEOUT_SECONDS" "$ROLLOUT_TIMEOUT_SECONDS"
set_command_env "HOTPOTQA_POLICY_CONCURRENCY" "$POLICY_CONCURRENCY"
set_command_env "HOTPOTQA_POLICY_RETRIES" "$POLICY_RETRIES"
set_command_env "HOTPOTQA_POLICY_MAX_TOKENS" "$POLICY_MAX_TOKENS"
replace_toml_array "train_seeds" "$(seed_array_literal "$TRAIN_SEED_START" "$TRAIN_SIZE")"
replace_toml_array "heldout_seeds" "$(seed_array_literal "$HELDOUT_SEED_START" "$HELDOUT_SIZE")"
...
echo "GEPA container: hotpotqa_container"
echo "GEPA dataset: train=$TRAIN_SIZE heldout=$HELDOUT_SIZE sampler=contiguous_random_seed_window"
echo "GEPA dataset_seed_starts: train=$TRAIN_SEED_START heldout=$HELDOUT_SEED_START"
